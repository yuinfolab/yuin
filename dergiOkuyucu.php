...
$pdfid = '';
if(isset($_GET['readMagId']) && !empty($_GET['readMagId']) && is_numeric($_GET['readMagId'])) {
    
    $pdfid = trim($_GET['readMagId']);
}
...
$tiraj = false;
if($stmt = $pdo->prepare("SELECT tiraj from magazine WHERE id = :id")) {
...
if(is_numeric($tiraj) && !is_bool($tiraj)) {
    
    $tiraj = (int)$tiraj;
    $tiraj++;
    
    if($stmt = $pdo->prepare("UPDATE magazine SET tiraj = :yt WHERE id = :id")) {
        
        // PDO parametrelerini ayarla
        $stmt->bindParam(":yt", $tiraj, PDO::PARAM_STR);
        $stmt->bindParam(":id", $pdfid, PDO::PARAM_STR);
        $stmt->execute();
    }
}
...
<head>
    ...
    <title>YUIN Dergi Okuyucusu</title>
    <link rel="stylesheet" href="js/pdf-flipbook-master/pdfjs/viewer.css"/>
    <link rel="stylesheet" href="js/pdf-flipbook-master/pdf-flip.css"/>
    <style>
        .se-pre-con {
            position: fixed;
            left: 0px;
            ...
            height: 100%;
            z-index: 9999;
            background: url(js/pdf-flipbook-master/pics/loader.gif) center no-repeat #fff;
        }
    </style>
</head>
...
<!-- outerContainer -->
<script>
    var DEFAULT_URL =  "https://yuin.yeditepe.edu.tr/getPdfData.php?pdfid=<?=$pdfid;?>";
</script>
</script>
<script src="js/pdf-flipbook-master/pdfjs/compatibility.js"></script>
<!-- This snippet is used in production (included from viewer.html) -->
<link rel="resource" type="application/l10n" href="js/pdf-flipbook-master/pdfjs/locale/locale.properties"/>
<script src="https://code.jquery.com/jquery-2.1.4.min.js"></script>
<script src="js/pdf-flipbook-master/turnjs/turn.min.js"></script>
<script src="js/pdf-flipbook-master/turnjs/zoom.min.js"></script>
<script src="js/pdf-flipbook-master/pdfjs/l10n.js"></script>
<script src="js/pdf-flipbook-master/pdfjs/build/pdf.js"></script>
<script src="js/pdf-flipbook-master/pdfjs/debugger.js"></script>
<script src="js/pdf-flipbook-master/pdfjs/viewer.js"></script>
<script src="js/pdf-flipbook-master/pdf-flip.js?v=1.1.1.1.1.1"></script>
